    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.selectXiangCe enumerateAssetsWithOptions:NSEnumerationReverse usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            if (result) {
                ...
                model.thImage=[UIImage imageWithCGImage:result.aspectRatioThumbnail];
...
-(void)selectButtonAction:(UIButton*)sender
{
    ...
    }else
    {
        if (self.selectImageArray.count>=5) {
            UIAlertView*alert=[[UIAlertView alloc]initWithTitle:bendihua(@"提示") message:bendihua(@"圖片最多可選擇五張") delegate:self cancelButtonTitle:bendihua(@"確定") otherButtonTitles:nil, nil];
            [alert show];
