...
-(instancetype)init
{
    if (self=[super init]) {
        ...
        self.titleLabel.text=bendihua(@"賬號中心");
    }
    return self;
}
-(void)configUI
...
-(void)makeLogInNameAndImageLabel
{
    ...
    {
        if ([showDict[@"type"] isEqualToString:@"email"]) {
            leftStr=bendihua(@"當前賬號:");
        }else
        {
            leftStr=bendihua(@"遊客賬號:");
        }
    ...
    if ([HTBindInfo haveBindOfficalAccount]) {
        self.topBackView.centerLabel.text=bendihua(@"已綁定");
        
        [self.topBackView.rightButton setTitle:bendihua(@"修改密碼") forState:(UIControlStateNormal)];

    }else
    {
        self.topBackView.centerLabel.text=bendihua(@"未綁定");
        [self.topBackView.rightButton setTitle:bendihua(@"點擊綁定") forState:(UIControlStateNormal)];
    }
     [self.topBackView.rightButton addTarget:self action:@selector(accountBind:) forControlEvents:(UIControlEventTouchUpInside)];
    self.topBackView.leftLabel.text=bendihua(@"官方賬號");
    ...
    if (![HTBindInfo haveBindOfficalAccount]) {
        ...
        [self.centerLabel setText:bendihua(@"綁定官方賬號,獲得價值10$大禮包") font:MXSetSysFont(9) color:CRedColor sizeToFit:YES];
    ...
    self.bottomBackView.leftLabel.text=bendihua(@"其他賬號");
    
    if([HTBindInfo haveBindThridAccount])
    {
        self.bottomBackView.centerLabel.text=bendihua(@"已綁定");
        [self.bottomBackView.rightButton setTitle:bendihua(@"已綁定") forState:(UIControlStateNormal)];
        
    }else
    {
    self.bottomBackView.centerLabel.text=bendihua(@"未綁定");
    [self.bottomBackView.rightButton setTitle:bendihua(@"點擊綁定") forState:(UIControlStateNormal)];
    ...
    [changeAccountButton setTitle:bendihua(@"切換賬號") font:MXSetSysFont(11) backColor:CRedColor corner:4];
...
{
    if ([self.topBackView.rightButton.currentTitle isEqualToString:bendihua(@"修改密碼")]) {
...
{
    if ([self.bottomBackView.rightButton.currentTitle isEqualToString:bendihua(@"已綁定")]) {
