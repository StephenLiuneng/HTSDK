...
-(instancetype)init
{
    if (self=[super init]) {
        ...
        self.titleLabel.text=bendihua(@"忘記密碼");
        [self configUI];
    }
    return self;
}
-(void)configUI
{
    ...
    self.emailTextfield.placeholder=bendihua(@"請輸入官方賬號註冊郵箱");
...
-(void)resetAction:(HTBaseButton*)sender
{
    if (![regex isValidateEmail:self.emailTextfield.text]) {
        ...
        [HTAlertView showAlertViewWithText:bendihua(@"請輸入正確的郵箱") com:^{}];
    }
    else
    {
        [HTprogressHUD showjuhuaText:bendihua(@"正在發送")];
        ...
        [HTNetWorking sendRequest:request ifSuccess:^(id response) {
            ...
            if ([response[@"code"] isEqualToNumber:@0]) {
                ...
                [HTAlertView showAlertViewWithText:bendihua(@"密碼已發送,請注意查收") com:nil];
            }else
            {
                [HTAlertView showAlertViewWithText:bendihua(@"郵箱不正確") com:nil];
            }
